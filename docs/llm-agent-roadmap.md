...
- LLM-backed agent using an OpenAI-compatible endpoint for planning, execution, and explanations.
- Autonomous capability to navigate databases, run scripts, and write artifacts (Python, Markdown, reports) with approvals.
...
- Define LLM integration spec: OpenAI-compatible endpoint, model routing, prompt templates, token limits, retry/backoff, and tool-call schema.
- Define database navigation contract: list schemas/tables, preview tables, safe SQL execution, query limits, and redaction rules.
- Define script execution contract: sandboxed Python runner, filesystem allowlist, resource caps, artifact outputs, and diff-before-apply.
...
- Implement LLM planner/operator using the OpenAI-compatible endpoint (prompt → plan → tool calls → summaries).
- Add toolkits for database navigation/querying and sandboxed Python execution.
- Add file authoring tools: Markdown report generation, safe file edits, and artifact publishing.
...
- System prompts for OpenAI-compatible models: planner/operator/explainer roles, tool-call format, and safety rules.
