...
def build_agent(instructions: str | None = None) -> Agent[AgentDeps, str]:
    system_instructions = instructions or (
        ...
        "Start by listing the project root with list_dir('.') and reading docs/brief.txt plus data/raw/*.csv and data/raw/*.json. "
        "Avoid search_text unless you already know which files to search. "
        "Compute row counts with query_db using COUNT(*) (do not invent column names). "
        "Delay run_python until data sources (DB tables + raw files) are enumerated. "
